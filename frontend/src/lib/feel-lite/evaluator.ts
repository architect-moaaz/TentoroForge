...
export function evaluate(node: ASTNode, ctx: Context): unknown {
  switch (node.type) {
    ...
    case "ComparisonExpression": {
      ...
      // Numeric comparison — coerce numeric STRINGS too so `age >= 18` with
      // age="9" compares 9 >= 18, not lexicographic "9" >= "18". Matches
      // fuzzyEqual (= / !=), the emitted runtime and the Python engine. Empty
