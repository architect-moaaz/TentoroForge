...
function collectBindings(node: any): Array<{ propPath: string; binding: string }> {
  ...
  const walk = (obj: any, prefix: string) => {
    ...
    // interpolation strings and are caught by the branches below.
          ...
          return (
            <li key={i}>
              <BindingControl
                ...
                onChange={(v) => {
                  // One format, one action. The old fork wrote an
                  // interpolation string for values that were already
                  // strings and the unrenderable object form for everything
                  // else; bindProp now emits the string in both cases.
                  dispatch({
                    type: "bindProp",
                    pageId: hit.pageId,
