...
export function BindingControl({ label, value, onChange, placeholder, pageId: pageIdProp }: BindingControlProps) {
  ...
  // Enter. Wiring it straight to `onChange` dispatched a bindProp PER
  // KEYSTROKE, and editor-store pushes one undo entry per dispatch and re-arms
  // the 500 ms autosave each time — typing `form.email` cost nine undo entries
  // and nine save arms, and every intermediate prefix (`f`, `fo`, ...) was
  // written into the schema as a real binding. That was survivable while the
  // prop became an inert object; now that a bind is a live interpolation, each
  // prefix is a binding the renderer actually tries to resolve.
  //
  // Commit-on-blur matches SizeField in StylePanel.tsx. The dropdown and
  // "create data source" paths are discrete choices and still commit
  // immediately; the effect re-syncs the draft whenever they (or an external
  // edit) change the committed value.
