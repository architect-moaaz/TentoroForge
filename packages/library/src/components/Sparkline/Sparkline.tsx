...
export function Sparkline({
  data, width = 100, height = 24, color, showDots = false,
}: SparklineProps) {
  // NOT a parameter default. `color = "currentColor"` only fires for
  // `undefined`, and the value that actually arrived was `""` — so the
  // polyline was stroked with an invalid paint and drew nothing at all.
  // See util/paint.
  const stroke = paintOr(color, "currentColor");
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} aria-hidden="true">
      <polyline fill="none" stroke={stroke} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" points={points.join(" ")} />
