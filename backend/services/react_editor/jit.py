...
`static/react-jit.mjs` bundles the page with the app's own esbuild against
the app's own node_modules, compiles the app's Tailwind over it, and runs
the page's `load()` against a sample-data stand-in for `@/sdk/server` built
from the Blueprint's entities (PREVIEW-004, PREVIEW-008). The result is one
script and one stylesheet, cached per page revision under
`.forge/editor/jit/`, that the editor puts in a same-origin iframe.
"""
...
def _cache_key(page_id: str, revision: str, params: dict, search: dict) -> str:
    h = hashlib.sha1()
    h.update(revision.encode())
    h.update(json.dumps([params, search], sort_keys=True).encode())
    # The shims are part of what is rendered; a change to them is a new build.
...
def build(project: Project, page_id: str, *, params: dict[str, str] | None = None,
    ...
    key = _cache_key(page_id, revision, params, search)
    ...
    out = {"js": result["js"], "css": result["css"], "revision": revision, "ms": int((time.monotonic() - t0) * 1000),
