...
export const dataEngine: DataEngine = {
  async run(source: unknown, ctx?: { request?: Request; user?: { id?: string; role?: string; email?: string } }) {
    ...
    try {
      ...
      // "$routeId" sentinel — a list source scoped to the current detail route
      // (e.g. price offers for /scans/[id]/results) resolves it to the URL id.
      // If no route id is present the filter is left as-is, so the query
      // matches nothing rather than leaking the unscoped list.
      if (ctx?.request && Object.values(filters).includes("$routeId")) {
        try {
          const rid = new URL(ctx.request.url).searchParams.get("id");
          if (rid) {
            for (const k of Object.keys(filters)) {
              if (filters[k] === "$routeId") filters[k] = rid;
            }
          }
