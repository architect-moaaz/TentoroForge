 ...
 *
 * `__AUTH_LAYOUT__` is replaced at generation time with the layout chosen by
...
const HEADLINE = "__AUTH_HEADLINE__";
const SUBHEAD = "__AUTH_SUBHEAD__";
...
const FORM_TITLE = "__AUTH_FORM_TITLE__";
const FORM_SUB = "__AUTH_FORM_SUB__";
...
// The form half of every panel layout (split-editorial, split-reversed,
// side-panel). Those layouts put ALL their visual structure in the brand
// aside — which is `hidden lg:block`, so under 1024px the aside vanishes
// and the form was left floating on a flat background with no surface,
// no brand and no edges. The card is what `centered-minimal` and
// `brand-wash` already give their forms; there was no reason the panel
// layouts should be the exception, and every narrow viewport paid for it.
// The lockup is `lg:hidden` on purpose: above lg the aside carries the
// brand, below it nothing else names the app.
