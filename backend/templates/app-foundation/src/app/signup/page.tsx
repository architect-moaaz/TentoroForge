 ...
 * (`__AUTH_LAYOUT__`), and the brand surface is painted from the app's OWN
...
function BrandPanel({ variant }: { variant: "full" | "panel" }) {
  return (
    ...
    >
      ...
      <div className="relative z-10 text-lg font-semibold text-white">__APP_NAME__</div>
      <div className="relative z-10 space-y-3">
        <h2 className={`font-semibold leading-tight text-white ${variant === "full" ? "text-4xl lg:text-5xl" : "text-3xl"}`}
          style={{ fontFamily: "var(--font-heading)" }}>
          Get started in minutes.
        </h2>
        <p className="max-w-md text-sm leading-relaxed text-white/85">Create your account to start using __APP_NAME__.</p>
      </div>
      <div className="relative z-10 text-xs text-white/70">© __APP_NAME__</div>
    </div>
  );
}
