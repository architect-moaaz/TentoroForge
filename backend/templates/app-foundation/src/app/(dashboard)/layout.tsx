...
// Route hierarchy for shell-rendered breadcrumbs. Emitted by
// services.page_nav.write_route_tree_contract. Pages whose schema already
// carries a Breadcrumb are flagged `owned_by_schema` and the shell skips
// them, so a page never shows two trails. Absent contract → no crumbs,
// which is exactly the pre-existing behaviour.
async function loadRouteTree(): Promise<Record<string, RouteNode>> {
  try {
    const rp = path.join(process.cwd(), "src", "contracts", "route-tree.json");
    const parsed = JSON.parse(await fs.readFile(rp, "utf8"));
    return (parsed?.routes ?? {}) as Record<string, RouteNode>;
  } catch {
    return {};
  }
}
...
function frameClass(density?: string): string {
  ...
  // Mobile step uses an ARBITRARY value (p-[8px]) on purpose: the numeric
  // scale (p-4/p-6) is token-derived per app, and an inflated spec scale
  // (e.g. "6" → 40px) eats most of a 375px viewport. Larger breakpoints
  // keep the scale so desktop density still follows the design tokens.
  if (density === "compact") return "mx-auto w-full max-w-[1600px] p-[8px] sm:p-4 lg:p-6";
  if (density === "spacious") return "mx-auto w-full max-w-[1560px] p-[8px] sm:p-8 lg:p-12";
  return "mx-auto w-full max-w-[1600px] p-[8px] sm:p-6 lg:p-8";
}
...
async function readShellAppName(): Promise<string | undefined> {
  ...
  // gets a real brand name instead of the "__APP_NAME__" placeholder.
  try {
...
async function loadNavProps(): Promise<NavProps> {
  ...
  try {
    ...
    if (sn?.props?.groups?.length) {
      ...
      if (topLevelAppName && (!props.appName || props.appName === "__APP_NAME__")) {
  ...
  return { groups, appName: topLevelAppName || "__APP_NAME__", ...rail };
}
