...
export function sampleServer(entities, roles) {
  ...
  // the sample user was nobody's parent (rafm22pm: "Edit Child shows 404",
  // 2026-09-26). The signed-in person is the account entity's first sample
  // row, whose id the first row of every owned entity points at.
  const account = entities.find((e) => e.account);
  const me = account ? rows[account.name][0] : null;
  const user = me
    ? { id: me.id, name: me.fullName || me.name || "Sample Person", email: me.email || "admin@example.com", role: roles[0] || "admin" }
    : { id: "sample-user", name: "Sample Admin", email: "admin@example.com", role: roles[0] || "admin" };
