...
def test_ensure_api_routes_fills_every_entity(tmp_path):
    ...
    (schema / "customer.ts").write_text('export const customers = pgTable("customers", { id: serial("id").primaryKey() });')
    (schema / "work_order.ts").write_text('export const workOrders = pgTable("work_orders", { id: uuid("id").primaryKey() });')
    (schema / "index.ts").write_text('export * from "./customer";')
    ...
    listing = (api / "customers" / "route.ts").read_text()
    ...
    assert "Number(params.id)" in (api / "customers" / "[id]" / "route.ts").read_text()
    assert "Number(params.id)" not in (api / "work-orders" / "[id]" / "route.ts").read_text()


def test_non_destructive_keeps_existing_routes(tmp_path):
    schema = tmp_path / "src" / "db" / "schema"
    schema.mkdir(parents=True)
    (schema / "customer.ts").write_text('export const customers = pgTable("customers", { id: serial("id") });')
    existing = tmp_path / "src" / "app" / "api" / "customers"
    existing.mkdir(parents=True)
    (existing / "route.ts").write_text("// custom LLM route — keep me")

    ensure_api_routes(tmp_path)
    assert (existing / "route.ts").read_text() == "// custom LLM route — keep me"
