...
def test_write_then_read(tmp_path):
    ...
    assert (tmp_path / "src" / "a.ts").read_text(encoding="utf-8") == "export const x = 1;"
...
def test_glob(tmp_path):
    ...
    (tmp_path / "src" / "b.ts").write_text("b", encoding="utf-8")
...
def test_edit_missing_string_is_soft_error(tmp_path):
    (tmp_path / "x.ts").write_text("hello", encoding="utf-8")
