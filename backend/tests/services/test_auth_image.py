...
def test_resolve_prefers_design_spec(tmp_path):
    ...
    (tmp_path / "src" / "contracts" / "design-spec.json").write_text(
        json.dumps({"imagery": {"loginBackground": "https://images.unsplash.com/photo-XYZ"}}))
...
def test_resolve_falls_back_to_domain_when_spec_null(tmp_path):
    (tmp_path / "src" / "contracts").mkdir(parents=True)
    (tmp_path / "src" / "contracts" / "design-spec.json").write_text(
        json.dumps({"imagery": {"loginBackground": None}}))
...
def test_substitutes_placeholder(tmp_path):
    ...
    (tmp_path / "src" / "app" / "login" / "page.tsx").write_text('<img src="__AUTH_IMAGE_URL__" />')
    n = _substitute_auth_image(tmp_path, "Project Management")
    assert n == 1
    assert "__AUTH_IMAGE_URL__" not in (tmp_path / "src" / "app" / "login" / "page.tsx").read_text()
    assert "images.unsplash.com" in (tmp_path / "src" / "app" / "login" / "page.tsx").read_text()
...
def test_auth_copy_falls_back_without_name(tmp_path):
    login = tmp_path / "src" / "app" / "login"
    login.mkdir(parents=True)
    (login / "page.tsx").write_text('<h2>__AUTH_HEADLINE__</h2><p>__AUTH_SUBHEAD__</p>')
