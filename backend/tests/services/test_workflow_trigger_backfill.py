...
def _app(tmp_path: Path, plan_workflows: list[dict], files: dict[str, dict]) -> Path:
    ...
    (contracts / "plan.json").write_text(json.dumps({"workflows": plan_workflows}))
    wf_dir = tmp_path / "workflows"
    wf_dir.mkdir()
    for name, doc in files.items():
        (wf_dir / name).write_text(json.dumps(doc))
    return tmp_path
...
def test_event_trigger_backfilled(tmp_path):
    ...
    doc = json.loads((root / "workflows" / "NotifyOnOrder.json").read_text())
...
def test_schedule_trigger_backfilled_via_name_match(tmp_path):
    ...
    assert backfill_workflow_triggers(str(root)) == 1
    doc = json.loads((root / "workflows" / "digest.json").read_text())
...
def test_manual_workflow_is_noop(tmp_path):
    ...
    assert backfill_workflow_triggers(str(root)) == 0
    assert "trigger" not in json.loads((root / "workflows" / "CreateTask.json").read_text())
...
def test_idempotent_second_run(tmp_path):
    ...
    assert backfill_workflow_triggers(str(root)) == 1
    first = (root / "workflows" / "NotifyOnOrder.json").read_text()
    assert backfill_workflow_triggers(str(root)) == 0
    assert (root / "workflows" / "NotifyOnOrder.json").read_text() == first
...
def test_missing_pieces_graceful(tmp_path, missing):
    if missing == "workflows":
        contracts = tmp_path / "src" / "contracts"
        contracts.mkdir(parents=True)
        (contracts / "plan.json").write_text(json.dumps({"workflows": []}))
    else:
