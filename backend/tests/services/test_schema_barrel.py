...
def _mk(tmp_path: Path):
    ...
    (d / "property.ts").write_text('export const properties = pgTable("properties", {});')
    (d / "user.ts").write_text('export const users = pgTable("users", {});')
    (d / "relations.ts").write_text('export const propertiesRelations = relations(properties, () => ({}));')
    return tmp_path
...
def test_builds_complete_barrel_and_removes_shadow(tmp_path):
    ...
    idx = (root / "src" / "db" / "schema" / "index.ts").read_text()
