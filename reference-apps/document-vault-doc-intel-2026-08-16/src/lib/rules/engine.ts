...
function checkValidation(
  ...
  // Min / max: for strings compare length, for numbers compare value.
  // Coerce numeric-looking strings to numbers first — a workflow template
  // like `{{call_paddleocr.confidence}}` resolves to the string "0.95",
  // and a numeric-range rule (both min AND max defined) is unambiguously
  // meant to check the value, not the string's character count.
  const isNumericRange =
    typeof config.min === "number" && typeof config.max === "number";
  let cmpValue: string | number = value as any;
...
export async function canAccessField(
  ...
  for (const rule of rules) {
    ...
    // Check role list.
    //
    // A `roles` list means the field REQUIRES one of those roles. It does
    // not mean "check the role if the caller happens to have one".
    //
    // This condition used to end in `&& user.role`, so a caller presenting
    // NO role skipped the check entirely and was allowed — the field was
    // exposed to exactly the callers least entitled to it. `filterFields`
    // feeds API responses, so an hr/admin-only column (salary, SSN, notes)
    // was served to unauthenticated requests. Fail CLOSED: absent role is
    // "not in the list", not "no opinion".
    if (config.roles && config.roles.length > 0) {
      const role = typeof user.role === "string" ? user.role.trim() : "";
      if (!role) {
        return {
          allowed: false,
          reason: `No role presented; ${modelName}.${fieldName} is restricted to: ${config.roles.join(", ")}`,
        };
      }
      if (!config.roles.includes(role)) {
        return { allowed: false, reason: `Role ${role} not in allowed list` };
      }
    }
...
async function applyTypedRules(
  ...
  // holds a value). Passing the whole rule set for a `{status: "processing"}`
  // update wrongly complained "Original filename is required" for every other
  // required rule the model had.
  try {
    const scoped = event === "update"
      ? Object.fromEntries(Object.entries(working).filter(([k]) => k in (working as any)))
      : working;
    // For updates we still want to catch bad values in the fields the caller
    // set, so pass only those keys. validateEntity iterates the union of
    // `Object.keys(entity)` and `ruledFields`, so restricting to just the
    // update payload keys prevents rule-driven visits to absent columns.
    const entityForValidation = event === "update" ? scoped : working;
    const vr = await validateEntity(modelName, entityForValidation, { onlySubmittedFields: event === "update" });
