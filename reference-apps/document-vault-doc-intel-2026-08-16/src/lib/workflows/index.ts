...
function _resolveRef(ref: unknown, ctx: WorkflowExecutionContext): unknown {
  ...
  if (ref.includes("{{")) {
    // Sole-template short-circuit: `{{expr}}` with nothing else around it
    // returns the RAW value (objects, arrays, numbers) so downstream
    // consumers like a jsonb db_update get the real object — otherwise
    // `{{ai_extract_fields.extracted_fields}}` would collapse to the
    // string "[object Object]". Concatenated templates ("id={{x.id}}")
    // still string-interpolate as before.
    const soleMatch = ref.match(/^\s*\{\{\s*([\w.[\]]+)\s*\}\}\s*$/);
    if (soleMatch) {
      const key = soleMatch[1] as string;
...
export function registerDefaultActions(): void {
  ...
  registerActionHandler("http_call", async (config, ctx) => {
    ...
    try {
      // Resolve template refs inside body/url. The handler used to send `body`
      // untouched, so a workflow-authored `body: {file_url: "{{fileUrl}}"}` was
      // shipped as the literal `{{fileUrl}}` string — the sidecar received a
      // non-URL and 500'd. Walk one level: strings go through _resolveRef,
      ...
      const authorHeaders =
        (config as any).headers && typeof (config as any).headers === "object"
          ? ((config as any).headers as Record<string, string>)
          : {};
