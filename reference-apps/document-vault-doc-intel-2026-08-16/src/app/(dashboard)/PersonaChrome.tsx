...
function findActivePersonaId(
  ...
  for (const p of personas) {
    const routes = [
      ...p.jobs.map((j) => j.route),
      ...(p.screens || []).map((s) => s.route),
    ].filter(Boolean);
...
export function PersonaChrome({
  ...
  return (
    <>
      ...
      <div className="mx-auto flex max-w-screen-2xl items-center justify-between gap-4 px-6 py-3">
        ...
        >
          {personas.map((persona) => {
            ...
            const first = jobs[0] || (persona.screens || [])[0];
      ...
      {activePersona && (activePersona.screens || []).length > 0 && (
        <div
        ...
        >
          ...
          >
            {(activePersona.screens || []).map((screen) => {
